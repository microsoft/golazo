...
> Public draft – integrates legacy FAQ transition guidance & downsides with a structured rollout.
...
| Coach                | Access to experienced internal/external coach (≥ first 4 weeks) | Can be borrowed from another team   |
...
| Psychological Safety | Blameless retros & VSM expectation                              | Essential for honest improvement    |

## Phase 1 (Weeks 1–3): Learn & Prepare

Goals: establish shared vocabulary & instrumentation.

- Kickoff presentation (overview + Q&A).
- Stand up board with columns/rails & WIP limits.
- Seed backlog (enough refined tickets for 1–2 weeks).
- Create core channels (Dev General, Review, Release, Social).
- Run first Planning + Retro (even if thin).
- Draft first design docs (paired creation to teach template).
- Start capturing baseline metrics.

## Phase 2 (Weeks 4–8): Full Trial

Goals: stabilize flow & reinforce habits.

- Enforce WIP limits rigorously.
- Require design doc + 2 signoffs before engineering (except true Swarm).
...
## Phase 3 (Weeks 9–12): Optimize & Decide
...
- Document any customizations with rationale + rollback criteria.
...
| Quality          | % time on Swarm + Interrupt            | ≤ 30% (10% Swarm typical) |
| Learning         | % tickets with design doc & 2 signoffs | ≥ 95%                     |
| Knowledge Spread | Distinct Shepherds per workstream      | Trending upward           |
| Morale           | Pulse survey: "I can get help quickly" | ≥ 4/5 average             |

## Handling Downsides & Concerns

| Concern                              | Response                                                                                                                         |
| ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| "Too much ceremony"                  | Ceremonies are intentionally short; measure time spent—should be a small fraction vs regained flow.                              |
| "Hard boundary with other teams"     | Use Ready gate + clear acceptance criteria to collaborate; share design docs early with dependents.                              |
